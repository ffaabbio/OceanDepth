...
void inv_print(const Inventaire* inv, int perles){
    puts("\n================= INVENTAIRE =================");
    printf("Harpon equipe : %-15s | Combi equipee : %-15s | Perles : %d\n",
           nom_harpon(inv->eq_harpon), nom_combi(inv->eq_combi), perles);

    int db,od; combi_stats(inv->eq_combi,&db,&od);
    int mi,ma,oc,di; harpon_stats(inv->eq_harpon,&mi,&ma,&oc,&di);
    printf("Stats : ATK %2d-%-2d | DEF +%-2d | O2 atk %d / tour %d\n",
    ...
    for (int i=0;i<INV_MAX;i++){
        ...
        if (it->kind==ITEM_CONS) {
            printf("[%d] %-16s x%-2d\n", i+1, nom_consommable(it->subtype), it->qty);
        } else if (it->kind==ITEM_HARPON) {
            printf("[%d] %-16s (harpon)\n", i+1, nom_harpon(it->subtype));
        } else if (it->kind==ITEM_COMBI) {
            printf("[%d] %-16s (combi)\n", i+1, nom_combi(it->subtype));
        } else {
            printf("[%d] Vide\n", i+1);
        }
    }
    puts("---------------------------------------------");
    puts("1-Utiliser objet  2-Equiper harpon  3-Equiper combi  4-Retour");
}

// ----- Utilisation consommable -----
static int appliquer_consommable(Plongeur* j, ConsommableType t){
    switch (t){
        case CONS_O2:   j->o2 = clampi(j->o2 + 40, 0, j->o2_max); puts("+40 Oxygene."); return 1;
        ...
        case CONS_ANTIDOTE: puts("Antidote utilise."); return 1; // simplifie
        default: return 0;
    }
}
